...
- shell scripts under `tools/validation/` are syntax-checked with `bash -n`; shell scripts under `scripts/` are also syntax-checked when that directory exists (conditional — absent in minimal/zip review environments)
